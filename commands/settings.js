        ...
        try {
            ...
            if (args[0] === "list") {
                const settingsEmbed = new EmbedBuilder()
                    ...
                    .addFields(
                        ...
                        },
                        {
                            ...
                            value: `<#${settings.update_channel_id}>`,
                            inline: false,
                        },
                        {
                            name: "🤖 Prefix",
                    ...
                    .setFooter({
                        text: `Community ID: ${settings.community_id}`,
                    })
                ...
                if (!targetChannel || !targetChannel.isTextBased()) {
                    return message.send(
                        "Please provide a valid text channel in this community!",
                    );
                }

                if (args[1] === settings.channel_id) {
                    return message.send(
                        `Counting channel is already: <#${args[1]}>`,
                    );
                }
                ...
                await targetChannel.send(
                    `📢 <#${newSettings.channel_id}> has been set as the counting channel for this community!`,
                );
                await message.send(
                    `Counting channel set to: <#${newSettings.channel_id}>`,
                );
                ...
                if (args[1] === settings.update_channel_id) {
                    return message.send(
                        `Bot updates channel is already: <#${args[1]}>`,
                    );
                }
                ...
                await targetChannel.send(
                    `📢 <#${newSettings.update_channel_id}> has been set as the bot updates channel for this community!`,
                );
                await message.send(
                    `Bot updates channel set to: <#${newSettings.update_channel_id}>`,
                );
            } else if (
                args[0] === "arithmetic" &&
                ...
                if (toggle) {
                    message.send(
                        `Arithmetic Mode has been enabled! Any fundamental mathematical operation will be considered as a count!`,
                    );
                } else {
...
Use \`${settings?.prefix ?? process.env.PREFIX}lb com\`, \`${settings?.prefix ?? process.env.PREFIX}lb com hardcore\` or \`${settings?.prefix ?? process.env.PREFIX}lb com total\` to view your community's rank.`);
                    ...
                    message.send(
                        `Leaderboard has been disabled! Your community will no longer be shown on the leaderboard!`,
                    );
                }
            } else if (
                args[0] === "numbersonly" &&
                ...
                if (toggle) {
                    message.send(
                        `Numbers Only Mode has been enabled! Any text that is not a number will be deleted (or reset the count if **hardcore mode** is **enabled**).`,
                    );
                } else {
                    message.send(
                        `Numbers Only Mode has been disabled! Texting while counting is allowed.`,
                    );
                }
            } else if (
                args[0] === "hardcore" &&
                ...
                if (count.current_count === 0) {
                    ...
                    if (toggle) {
                        message.send(
                            `Hardcore Mode has been enabled! Any number that is incorrect will reset the counting.`,
                        );
                    } else {
...
If you wish to continue, please type \`${settings?.prefix ?? process.env.PREFIX}settings reset <CommunityID>\``);
                ...
                return await message.send(
                    "The count has successfully been reset!",
                );
            } else if (args[0] === "prefix" && args[1]) {
                if (args[1].length != 1) {
                    return message.send(
                        "Prefixes can only be 1 character long.",
                    );
                }
                await client.db.query(
                    `
                ...
                return await message.send(
                    `The prefix has successfully been set to \`${args[1]}\``,
                );
            } else {
                const settingsEmbed = new EmbedBuilder()
                    ...
                    .addFields(
                        {
                            ...
                            value: `\`${settings?.prefix ?? process.env.PREFIX}settings list\` - View settings`,
                            inline: false,
                        },
                        {
                            name: "📍 Change Channel",
                            value: `\`${settings?.prefix ?? process.env.PREFIX}settings channel <ID/#channel>\` - Change counting channel`,
                            inline: false,
                        },
                        {
                            name: "🧮 Arithmetic Mode",
                            value: `\`${settings?.prefix ?? process.env.PREFIX}settings arithmetic <disable/enable>\` - Toggle maths operations`,
                            inline: false,
                        },
                        {
                            name: "🏆 Leaderboard",
                            value: `\`${settings?.prefix ?? process.env.PREFIX}settings lb <disable/enable>\` - Toggle public community leaderboard`,
                            inline: false,
                        },
                        {
                            name: "🔢 Numbers Only Mode",
                            value: `\`${settings?.prefix ?? process.env.PREFIX}settings numbersonly <disable/enable>\` - Toggle chat while counting`,
                            inline: false,
                        },
                        {
                            name: "🔥 Hardcore Mode",
                            value: `\`${settings?.prefix ?? process.env.PREFIX}settings hardcore <disable/enable>\` - Toggle reset at incorrect number (must be 0 to initiate)`,
                            inline: false,
                        },
                        {
                            name: "0️⃣ Reset Count",
                            value: `\`${settings?.prefix ?? process.env.PREFIX}settings reset\` - Reset the counter`,
                            inline: false,
                        },
                        {
                            name: "🤖 Bot Updates Channel",
                            value: `\`${settings?.prefix ?? process.env.PREFIX}settings updatechannel <ID/#channel>\` - Change the bot updates channel`,
                            inline: false,
                        },
                        {
                            name: "🤖 Prefix",
                            value: `\`${settings?.prefix ?? process.env.PREFIX}settings prefix <prefix>\` - Set a new prefix`,
                            inline: false,
                        },
                    )
                    .setFooter({
